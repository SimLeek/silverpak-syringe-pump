...
class ControllerWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.motor=MotorData('1')
        self.motordict={'1': self.motor}

        self.parse_xml('syringe_pump_data.xml')

        fail_text= "WARNING: No silverpak found, entering testing mode.\n"
        try:
            if not self.motor.motor.findAndConnect(): #no motor found
    ...
    def init_motor(self):
        ...
        self.motor.motor.sendRawCommand("/"+self.motor.char+"Z10000R")
        while not self.motor.motor.isReady():
            pass
        #Acceleration. Needed for motor to actually move.
        self.motor.motor.sendRawCommand("/"+self.motor.char+"L5000R")
        while not self.motor.motor.isReady():
            pass
        #Velocity. Needs to be set low for motor to move without slipping.
        self.motor.motor.sendRawCommand("/"+self.motor.char+"V200000R")
        while not self.motor.motor.isReady():
            pass
...
#        pass
    ...
    def parse_xml(self, filename):
        ...
        try:
            ...
            for child in root:
                if child.tag=='mL_per_rad':
                    self.motor.mL_per_rad=float(child.text)
                elif child.tag=='pos_per_rad':
                    self.motor.motor_position_per_rad=float(child.text)
        ...
        if not hasattr(self,'mL_per_rad'):
            self.motor.mL_per_rad=0.016631691553103064#found experimentally
            self.xml_good=False
        if not hasattr(self,'pos_per_rad'):
            self.motor.motor_position_per_rad=8156.69083345965#found experimentally(on free motor)
            self.xml_good=False
    ...
    def write_xml(self,filename):
        ...
        mL_per_rad.text=str(self.motor.mL_per_rad)
        pos_per_rad=ET.SubElement(root, 'pos_per_rad')
        pos_per_rad.text=str(self.motor.motor_position_per_rad)
    ...
    def select_pump(self, text):
        ...
        try:
            self.motor=self.motordict[pump_char]
        except KeyError: #motor does not exist
            self.motor=MotorData('1')
            self.motordict[pump_char]=self.motor
    ...
    def no_max(self):
        ...
        if self.motor.is_max_set:
            ...
            self.motor.motor.sendRawCommand("/"+self.motor.char+"z"+str(self.motor.motor_position)+"R")
    ...
    def set_max(self):
        ...
        if not self.motor.is_max_set:
            ...
            self.motor.motor.sendRawCommand("/"+self.motor.char+"z0R")
   ...
   #-------------------------#
    ...
    def checkVelocity(self):
        ...
        """
        pos1txt=self.motor.motor.sendRawCommand("/"+self.motor.char+"?0")
        t1=time.clock()
        pos2txt=self.motor.motor.sendRawCommand("/"+self.motor.char+"?0")
        t2=time.clock()
        ...
        vReptxt=self.motor.motor.sendRawCommand("/"+self.motor.char+"?2")
    ...
    def checkStatus(self):
        ...
        motor_name=self.motor.motor.sendRawCommand("/"+self.motor.char+"&")
    ...
    def getPosition(self):
        ...
        """
        txt=self.motor.motor.sendRawCommand("/"+self.motor.char+"?0")
        #print(str(txt))
    ...
    def stop(self):
        """Stops the motor."""
        self.motor.motor.sendRawCommand("/"+self.motor.char+"TR")
    ...
    def handlePump(self):
        ...
        #Send!
        exe="/"+self.motor.char+"gV"+str(int(pull_vel))+"A"+str(int(pos2))+top_wait_string+"V"+str(int(push_vel))+"A"+str(int(pos1))+bottom_wait_string+"G"+str(int(no_pumps))+"R"
        print(exe)
        self.motor.motor.sendRawCommand(exe)
    ...
    def handleInject(self):
        ...
        #set velocity
        self.motor.motor.sendRawCommand("/"+self.motor.char+"V"+str(int(vel))+"R")
        ...
        #Inject!
        self.motor.motor.sendRawCommand("/"+self.motor.char+"A"+str(int(self.motor.motor_position))+"R")
